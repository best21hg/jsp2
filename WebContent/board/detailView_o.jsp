...
<!-- 출력 포맷 사용하기 위한 태그라이브러리 설정 라이브러리 -->
...
<!-- 				<li>글 비밀번호</li>댓글 삭제 : 1)로그인한 사용자가 본인글 삭제 또는 2)글 비밀번호 사용				
			...
			<li>
				<ul style="display: flex;">
					<li>
						...
						<li styl	e="align-self: center;margin-bottom: 20px;">
	...
	function modalSet(vfunc) {
		...
		document.forms[1].func.value=vfunc;	//삭제 : ,수정: 2로 기능설정하기
	}
	
	//메인글 수정
	function modalOK() {
		if(document.forms[1].func.value==='2'){ //수정
			document.froms[1].action ='updateAction.jsp'
			document.froms[1].submit();
		}else if(document.forms[1].func.value==='1'){
			document.froms[1].action ='deleteAction.jsp'
			document.froms[1].submit();
		} else{
			alert('잘못된 동작입니다.!');
		}
